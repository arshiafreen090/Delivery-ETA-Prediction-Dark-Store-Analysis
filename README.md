...
- Delivery ETA is influenced by operational, geographic, temporal, and environmental factors.
- Haversine distance provides a useful geographic feature for delivery prediction.
- Leakage-safe preprocessing was implemented through `ColumnTransformer` and `Pipeline`.
- Tuned XGBoost achieved an MAE of approximately **3 minutes** on the test set.
- SHAP provides an interpretable view of model behavior.
- Dark-store proximity can be analyzed independently using geographic nearest-neighbor search.
- A food-delivery ETA model should not automatically be reused for quick-commerce ETA without validating the underlying delivery process and target distribution.
